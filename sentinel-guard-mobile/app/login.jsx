    ...
    <KeyboardAvoidingView style={{ flex: 1, backgroundColor: colors.bg }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      <ScrollView contentContainerStyle={{ flexGrow: 1, justifyContent: 'center', padding: spacing.lg }}>
        ...
        <View style={{ alignItems: 'center', marginBottom: 40 }}>
          <View style={{ width: 60, height: 60, borderRadius: 16, backgroundColor: colors.accentBg, borderWidth: 1, borderColor: colors.accentB, alignItems: 'center', justifyContent: 'center', marginBottom: 16 }}>
        ...
        <View style={{ backgroundColor: colors.bg2, borderWidth: 1, borderColor: colors.border, borderRadius: radius.xl, padding: spacing.lg }}>
          ...
          <TextInput
            ...
            autoCapitalize="none"
            style={{ backgroundColor: colors.bg3, borderWidth: 1, borderColor: colors.border2, borderRadius: radius.md, padding: 11, color: colors.text, fontSize: 14, marginBottom: 14 }}
          ...
          <TextInput
            ...
            secureTextEntry
            style={{ backgroundColor: colors.bg3, borderWidth: 1, borderColor: colors.border2, borderRadius: radius.md, padding: 11, color: colors.text, fontSize: 14, marginBottom: 20 }}
            placeholderTextColor={colors.text3}
            placeholder="••••••••"
          />

          {!!error && (
            <Text style={{ fontSize: 12, color: colors.red, textAlign: 'center', marginBottom: 12 }}>{error}</Text>
          )}

          <TouchableOpacity
            onPress={handleLogin}
            disabled={loading}
            style={{ backgroundColor: colors.accent, borderRadius: radius.md, padding: 13, alignItems: 'center', flexDirection: 'row', justifyContent: 'center', gap: 8, opacity: loading ? 0.7 : 1 }}
          >
          ...
          <Text style={{ fontSize: 11, color: colors.text3, textAlign: 'center', marginTop: 12, fontFamily: colors.mono }}>
            Role assigned automatically on authentication
          </Text>
        </View>

        {/* Dev hint */}
        <View style={{ marginTop: 16, backgroundColor: colors.bg3, borderRadius: radius.md, padding: 12 }}>
          <Text style={{ fontSize: 11, color: colors.text3, fontFamily: colors.mono, textAlign: 'center' }}>
